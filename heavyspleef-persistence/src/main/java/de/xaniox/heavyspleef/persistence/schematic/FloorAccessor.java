			...
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++) {
					for (int z = 0; z < length; z++) {
						...
						BaseBlock block = clipboard.getFullBlock(minPoint.add(BlockVector3.at(x, y, z)));

						if (block.getBlockType().getLegacyId() > Byte.MAX_VALUE - Byte.MIN_VALUE) {
							if (addBlocks == null) {
								addBlocks = new byte[(blocks.length >> 1) + 1];
							}

							if ((index & 1) == 0) {
								addBlocks[index >> 1] = (byte) (addBlocks[index >> 1] & 0xF0 | (block.getBlockType().getLegacyId() >> 8) & 0x0F);
							} else {
								addBlocks[index >> 1] = (byte) (addBlocks[index >> 1] & 0x0F | (block.getBlockType().getLegacyId() >> 4) & 0xF0);
							}
						}

						blocks[index] = (byte)(block.getBlockType().getLegacyId() & 0xFF);
						data[index] = (byte)(block.getBlockType().getLegacyData() & 0xFF);
	...
	public Floor read(InputStream in) throws IOException, CodecException {
		...
		try {
			...
			if (childs.containsKey("origin")) {
				...
				origin = BlockVector3.at(originX, originY, originZ);
			}
			...
			Map<BlockVector3, Map<String, Tag>> tileEntitiesMap = new HashMap<BlockVector3, Map<String, Tag>>();
			...
			for (int x = 0; x < width; ++x) {
				for (int y = 0; y < height; ++y) {
					for (int z = 0; z < length; ++z) {
						...
						Material blockMat = convertMaterial(blocks[index], blockData[index]);
						BlockType bt = new BlockType(blockMat.toString());

						BaseBlock block = bt.getDefaultState().toBaseBlock();
